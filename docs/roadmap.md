...
> 任何 AI agent / 自动化脚本看到本节时应**优先**更新这里的状态，不要跑去 git log 反推。状态由 roadmap 维护者手工或 `docs/releases/M{N}-*.md` 自动摘录推动更新。

| 阶段 | 当前状态 | 启动日期 | 通过 release-doc | 通过日期 | 备注 |
| --- | --- | --- | --- | --- | --- |
| 阶段 0 文档基线 | ✅ 已完成 | 2026-07-22 | — | 2026-07-23 | ADR-0001 / ADR-0002 / roadmap 落地 |
| 阶段 1 工程起点 | ⏳ 待开始 | — | — | — | 入口条件见 §4.2 |
| 阶段 2 Git CLI 化 | ⏳ 待开始 | — | — | — | 与阶段 1 互锁 |
| 阶段 3 大仓库体验 | ⏳ 待开始 | — | — | — | 依赖 L / XL / XXL 样本 |
| 阶段 4 游戏工程特性 | ⏳ 待开始 | — | — | — | xgit-server 协议 v0.1 待另起 ADR |
| 阶段 5 UI / UX 与风控 | ⏳ 待开始 | — | — | — | — |
| v1.0 候选 | ⏳ 终点 | — | — | — | 详见 §9 |

> 状态字符：用 ✅ 完成、🚧 进行中、⏳ 待开始、⚠️ 偏离、🚫 弃用。**任何 AI agent 在动本节前先读最后两行 git log + 对应 `docs/releases/M{N}-*.md` 的最新版本日期**。

## 13. 阶段推进流程（执行约定）

后续提交工作**必须**按本路线图推进，AGENTS.md §8 已把"不按 roadmap 推进"列为反模式。具体流程：

1. **进阶段前**：先对照"§N.M 进入门槛"逐条核对，缺一条就停下来补，不开新阶段代码
2. **阶段内**：完成"§N.M 步骤"全部子项；中间发现旧阶段遗漏，倒回旧阶段补，不在本阶段偷偷加
3. **退阶段前**：先做 §N.退出门槛的所有验证（命令原始输出 / p50/p95 / UI 截图 / 内存快照）；把验证结果与命令原文写进对应 `docs/releases/M{N}-*.md`
4. **跨阶段调整**：必须在本路线图更新 + AGENTS.md §7 决策记录 + 必要时另起 ADR 修订**同步落地**；不允许"代码先动，文档后补"
5. **release-doc 命名**：见 §10。每个阶段的发布说明是阶段退出门槛的**唯一证据**，不是事后总结
6. **与 ADR 的关系**：
   - 路线图阶段本身不另起 ADR；改动路线图通过 PR + 决策记录完成
   - 阶段内出现的新约束按"是否影响 ADR-0001 七条硬约束"决定是否另起 ADR
   - 各调研文档只在路线图反向引用，不被独立 PR 之外的工作流修改

> **任何"凭直觉跳过"或"先做再补"的请求，先暂停，等路线图更新后再继续。**

### 13.1 防跑偏检查清单（每次提交前自查）

按本路线图推进工作时，每次 commit / PR / "我开始动了"前自问：

| # | 检查项 | 答案需写在 | 否决条件 |
| --- | --- | --- | --- |
| 1 | 当前在哪个阶段？是否与本节状态表一致？ | §12.5 状态表 | 不符 → 暂停，先更新 §12.5 |
| 2 | 阶段进入门槛是否全部满足？ | 对应 §N.M 进入门槛 | 缺一条 → 停在 §N，不开 §N+1 |
| 3 | 动作是否属于本阶段？是否在"§N.M 步骤" / "§N.M 不做"清单里？ | 对应阶段 | 跨阶段 / 范围内不该做的 → 提 ADR 或倒退 |
| 4 | 是否会影响 ADR-0001 七条硬约束？ | ADR-0001 | 影响 → 先起 ADR 修订 |
| 5 | 是否需要更新 §12.5 / 其它阶段状态？ | §12.5 | 漏改 → 提交前补 |
| 6 | 退阶段时 release-doc 名称是否按 §10 命名？ | `docs/releases/M{N}-*.md` | 不是 → 改名 |
| 7 | 退阶段时是否附带"命令原始输出 / 数据快照 / 截图"？ | release-doc | 没记录 → 不算退阶段 |
| 8 | commit message 是否按 AGENTS.md §5.2 / §8 写？ | commit | 含 emoji 占位 / "AI generated" 等 → 改 |
| 9 | 文件组织是否匹配 AGENTS.md §6 目录树？ | 文件系统 | 落进错误目录 → 移到 `docs/releases/` 等正确位置 |
| 10 | 涉及 LICENSE / CI / 安全配置的修改是否走了"先和用户确认"？ | AGENTS.md §5.3 | 没确认 → 暂停 |

> 任何一条回答"否 / 不知道 / 想跳过"，对应行的"否决条件"会触发自动 / 手动暂停。**这是反模式自检，不是仪式**。
